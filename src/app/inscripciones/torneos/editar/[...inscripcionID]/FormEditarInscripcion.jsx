...
const FormEditarInscripcion = ({
  ...
  const {
    ...
    loading,
    error,
  } = useFetch(`torneo/${inscripcionData.inscripcion.torneo._id}`);
  const [shouldSubmit, setShouldSubmit] = useState(false);
  const [pruebasSelected, setPruebasSelected] = useState([]);

  useEffect(() => {
    setPruebasSelected(
      inscripcionData.inscripcion.pruebasInscripto.map((pruebaInscripto) => {
        return {
    ...
    if (valid) {
      try {
        const requests = pruebasSelected.map(async (prueba) => {
          if (
            inscripcionData.inscripcion.pruebasInscripto.some(
              (p) => p._id === prueba.id
            )
          ) {
            const { data } = await axios.put(
              `${process.env.NEXT_PUBLIC_URL_API}/pruebas_atleta/${prueba.id}`,
              prueba
            );
      ...
      <PruebasLogic
        ...
        pruebas={torneoData.torneo.pruebasDisponibles.filter((p) => {
          if (!p.categorias) {
            ...
            p.categorias.some(
              (c) => c._id === inscripcionData.inscripcion.categoria._id
            )
          ) {
            return p;
          }
      ...
      <div className="flex flex-col gap-4 items-left justify-start">
        {(torneoData.linkPagoFederados || torneoData.linkPagoNoFederados) && (
          <h3 className="text-text font-text font-semibold p-2">
            Por favor, guarde el comprobante de pago. En caso de no tener los
            medios de pagos aceptados, puede contactar por este mail:
            master1963@live.com.ar.
          </h3>
        )}
