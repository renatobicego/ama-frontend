...
const FormEditarTorneo = ({ torneo }) => {
  const [data, setData] = useState({
    ...
    categoriasDisponibles: torneo.categoriasDisponibles.map(
    ...
    linkPagoNoFederados: torneo.linkPagoNoFederados
      ? torneo.linkPagoNoFederados
      : "",
    requerirComprobante: torneo.requerirComprobante,
    precioInscripcion: {
      cada: torneo.precioInscripcion?.cada,
      ama: torneo.precioInscripcion?.ama,
    },
  });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (resultados) {
        data.resultados = await subirArchivoFirebase(
          resultados,
          "archivos/resultadosTorneos/"
        );
      }
      const res = await axios.put(
        `${process.env.NEXT_PUBLIC_URL_API}/torneo/${torneo._id}`,
        data,
        {
          headers: {
  ...
  const handleDelete = async () => {
    const shouldDelete = window.confirm(
      "¿Estás seguro de que quieres borrar el torneo?"
    );
  ...
  return (
    <>
      ...
      <button
        ...
        className="btn-primary bg-primary2 text-white"
      >
        Borrar Torneo
      </button>
    </>
