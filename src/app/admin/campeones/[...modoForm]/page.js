...
const PublicarCampeon = ({ params }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data: dataCampeon } = await axios.get(
          `${process.env.NEXT_PUBLIC_URL_API}/campeones/${modoForm[1]}`
        );
        setData({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (modoForm[0] === "publicar") {
        data.img = await subirArchivoFirebase(
          await comprimirArchivos(img, "jpeg"),
          "images/campeones/"
        );
        res = await axios.post(
          `${process.env.NEXT_PUBLIC_URL_API}/campeones`,
          data,
          {
            headers: {
        ...
        res = await axios.put(
          `${process.env.NEXT_PUBLIC_URL_API}/campeones/${data.id}`,
          data,
          {
            headers: {
              "x-token": session.user.token,
        ...
        throw new Error(
          "Error. Por favor, visite este sitio de nuevo y no cambie la url"
        );
      }
    ...
    <main className="pt-[17vh] lg:pt-44 2xl:pt-52 pb-20">
      <section className="size-section flex flex-col items-start gap-4 md:gap-8 xl:mt-6">
        ...
        <h2 className="text-title title-section text-left">
          {modoForm[0] === "editar"
            ? "Editar Campeón Nacional"
            : "Publicar Campeón Nacional"}
        </h2>
